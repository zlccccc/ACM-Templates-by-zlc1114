...
int solve() {
    ...
    int res=0;
    map<vector<int>,int> cliques;  // 这个是后面要算value所以搞得; O(n)
    // 点数1e5; 边数3e5; map跑了4.5s; vec跑了2.5s
    {
        ...
        while (Q.size()) {
            ...
            if (vis[x]) continue;
            vis[x]=1;
            vector<int> current_clique;  // 本身就是有序的
            current_clique.reserve(4); 
            current_clique.emplace_back(x);

            vector<pii> current_edge;
            for (auto &p:edge[x]) current_edge.emplace_back(p);
            function<void(int,int)> solve_subset=[&](int id,int sum) {  // 计算含有x的subset
                if (id==current_edge.size()) {
                    // solve
                    max_(res,sum);
                    auto newclique=current_clique;
    ...
    {
        map<vector<int>,int> link;  // map<vector>最好不要用; long long来哈希一下会快很多
