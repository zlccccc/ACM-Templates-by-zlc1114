...
int query(int x,int l,int r,bool flag,int &vmin,int &vmax,int L,int R){//flag:-> (top->bottom yes)
//    printf("%d %d %d %d %d %d\n",x,l,r, L, R,flag);
    if (l<=L&&R<=r){
//        update(x,val);//上面好像可去 
        vmin=mn[x];
    ...
    if(mid>=l) ret=max(ret,query(x<<1,l,r,flag,mn1,mx1,L,mid));
    if(r>mid) ret=max(ret,query(x<<1|1,l,r,flag,mn2,mx2,mid+1,R));
...
int Query(int x,int y){
    ...
    while(top[x]!=top[y]){
        if (dep[top[x]]>dep[top[y]]){
            ret=max(ret,query(1,id[top[x]],id[x],0,vmin,vmax,1,tot));
            ...
            ret=max(ret,query(1,id[top[y]],id[y],1,vmin,vmax,1,tot));
    ...
    if (dep[x]>dep[y]){
        ret=max(ret,query(1,id[y],id[x],0,vmin,vmax,1,tot));
        ret=max(ret,vmax-mnx);
        mxx=max(mxx,vmax);
        mnx=min(mnx,vmin);
    }
    else{
        ret=max(ret,query(1,id[x],id[y],1,vmin,vmax,1,tot));
        ret=max(ret,mxy-vmin);
        mxy=max(mxy,vmax);
        mny=min(mny,vmin);
    }
