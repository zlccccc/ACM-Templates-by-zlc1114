...
int main() {
	...
	scanf("%d%d",&k,&n);
	maxflow::init(n+k+2);
	int s=n+k+1,t=n+k+2,all=0;
	FOR(i,1,k) {
		int v; 
		scanf("%d",&v); all+=v;
		maxflow::addedge(s,i,v);
	} FOR(i,1,n){
		int p;
		scanf("%d",&p);
		while (p--){
			int t; scanf("%d",&t);
			maxflow::addedge(t,i+k,1);
		} maxflow::addedge(i+k,t,1);
	} if (maxflow::maxflow(s,t,n+k+2)!=all) puts("No Solution!");
	else{
		FOR(i,1,k) {
			printf("%d: ",i);
			for (int v=maxflow::head[i]; ~v; v=maxflow::edge[v].next)
				if (!maxflow::edge[v].cap&&maxflow::edge[v].to!=s){
				printf("%d ",maxflow::edge[v].to-k);
			}puts("");
		}
	}
}
