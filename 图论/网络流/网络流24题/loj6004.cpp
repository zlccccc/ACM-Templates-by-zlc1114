...
int main() {
	scanf("%d%d",&m,&n);
	maxflow::init(n+m+2);
	int s=n+m+1,t=n+m+2,i,k,j,all=0;
	FOR(i,1,m) scanf("%d",&k),maxflow::addedge(s,i,k),all+=k;
	FOR(i,1,n) scanf("%d",&k),maxflow::addedge(i+m,t,k);
	FOR(i,1,m) FOR(j,1,n) id[i][j]=maxflow::addedge(i,j+m,1);
	...
	if (x!=all) puts("0");
		...
		FOR(i,1,m){
			FOR(j,1,n){
				if (maxflow::edge[id[i][j]].cap==0) printf("%d ",j);
			}puts("");
		}
	}
}
