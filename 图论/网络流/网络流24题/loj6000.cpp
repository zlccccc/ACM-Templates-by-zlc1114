...
int main() {
	...
	maxflow::init(n+2);
	int s=n+1,t=n+2,i;
	FOR(i,1,m) maxflow::addedge(s,i,1);
	FOR(i,m+1,n) maxflow::addedge(i,t,1);
	int u,v;
	while (~scanf("%d%d",&u,&v)&&~u&&~v){
		id[++C]=maxflow::addedge(u,v,1);
	} int x=maxflow::maxflow(s,t,n+2);
	// if (!x) puts("No Solution!");
	// else{
		printf("%d\n",x);
	// 	FOR(i,1,C) if (maxflow::edge[id[i]].cap==0){
	// 		printf("%d %d\n",maxflow::edge[id[i]+1].to,maxflow::edge[id[i]].to);
	// 	}
	// }
}
/*
