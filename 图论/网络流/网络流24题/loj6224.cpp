...
inline void addedge(int x,int y,int flow,int cost) {
	// printf("%d - %d : %d %d\n",x,y,flow,cost);
	mincostflow::addedge(x,y,flow,cost);
}
int a,b,P,Q;
...
int main() {
	...
	FOR(i,0,P){
		FOR(j,1,Q) {
			int k; scanf("%d",&k);
			addedge(getid(i,j-1),getid(i,j),1,-k);
			addedge(getid(i,j-1),getid(i,j),INF,0);
		}
	}
	FOR(j,0,Q){
		FOR(i,1,P) {
			int k; scanf("%d",&k);
			addedge(getid(i-1,j),getid(i,j),1,-k);
			addedge(getid(i-1,j),getid(i,j),INF,0);
		}
	}
	FOR(i,1,a){
		int k,x,y;
	...
	FOR(i,1,b){
		int r,x,y;
		scanf("%d%d%d",&r,&x,&y);
		addedge(getid(x,y),t,r,0);
	}
	int ans=-mincostflow::mincostflow(s,t,t).second;
	printf("%d\n",ans);
}
/*
*/
