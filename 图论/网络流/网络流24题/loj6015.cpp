...
int main() {
	...
	FOR(i,1,m){
		scanf("%d",&H[i]);
		int k,t; scanf("%d",&k);
		while (k--) scanf("%d",&t),t+=2,V[i].push_back(t);
	} int all=0;
	maxflow::init(n*1000+2);
	addedge(1,4,INF); addedge(3,2,INF);
	FOR(T,1,1000){
		FOR(i,1,m){
			int x=V[i][(T-1)%V[i].size()],y=V[i][T%V[i].size()];
			x=T*n+x+2-n; y=T*n+y+2;
			addedge(x,y,H[i]);
		} FOR(i,1,n) addedge(T*n+i+2-n,T*n+i+2,INF);
		addedge(1,T*n+4,INF);
		addedge(T*n+3,2,INF);
		all+=maxflow::maxflow(1,2,T*n+n+2);
		// printf("time=%d : %d\n",T,all);
		if (all>=k) break;
		//什么垃圾题。。读错两次
	} if (T<=1000) printf("%d\n",T);
