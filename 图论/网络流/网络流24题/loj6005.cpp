...
} int A[maxn],p[maxn];//p:length
// ½¨·Ö²ãÍ¼
// ´ó¸Å¾ÍÊÇ, Ä³¸öÎ»ÖÃÖ»ÔÚÒ»²ã
int main() {
	scanf("%d",&n); int i,j,H=0;
	FOR(i,1,n) scanf("%d",&A[i]);
	FOR(i,1,n) {
		p[i]=1;
		FOR(j,1,i-1) if (A[i]>=A[j]) max_(p[i],p[j]+1);
		max_(H,p[i]);
	} printf("%d\n",H);
	int s=n+n+1,t=n+n+2;
	maxflow::init(n+n+2);
	FOR(i,1,n) maxflow::addedge(i,i+n,1);
	FOR(i,1,n) if (p[i]==1) maxflow::addedge(s,i,INF);
	FOR(i,1,n) if (p[i]==H) maxflow::addedge(i+n,t,INF);
	FOR(i,1,n) FOR(j,1,i-1) if (A[i]>=A[j]&&p[i]==p[j]+1)
		maxflow::addedge(j+n,i,1);
	int ans;
	printf("%d\n",ans=maxflow::maxflow(s,t,n+n+2));
	maxflow::addedge(1,1+n,INF);
	maxflow::addedge(n,n+n,INF);
	int nxt=maxflow::maxflow(s,t,n+n+2);
	if (nxt<INF/2) ans+=nxt;
	printf("%d\n",ans);
}
/*
5
