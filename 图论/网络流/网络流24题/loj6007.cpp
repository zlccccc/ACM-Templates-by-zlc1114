...
int main() {
	...
	scanf("%d%d",&n,&m);
	maxflow::init(n*m+2);
	int s=n*m+1,t=n*m+2,all=0;
	FOR(i,1,n) FOR(j,1,m){
		int k; scanf("%d",&k);
		all+=k;
		if ((i+j)&1){
			maxflow::addedge(s,getid(i,j),k);
			if (i!=1) maxflow::addedge(getid(i,j),getid(i-1,j),INF);
			if (j!=1) maxflow::addedge(getid(i,j),getid(i,j-1),INF);
		} else {
			maxflow::addedge(getid(i,j),t,k);
			if (i!=1) maxflow::addedge(getid(i-1,j),getid(i,j),INF);
			if (j!=1) maxflow::addedge(getid(i,j-1),getid(i,j),INF);
		}
	} printf("%d\n",all-maxflow::maxflow(s,t,n*m+2));
}
/*
*/
