...
int cnt,k;
void dfs(int x){
	for (int i=mincostflow::head[x]; ~i; i=mincostflow::edge[i].next) if (!(i&1)){
		while (mincostflow::edge[i^1].cap) {
			mincostflow::edge[i^1].cap--;
			int nxt=mincostflow::edge[i].to;
			if (n*m+1<=x&&x<=n*m*2){
				if (nxt==x-n*m+1&&m!=1) printf("%d 1\n",cnt);
				else printf("%d 0\n",cnt);
			} dfs(nxt); return;
			// printf("(%d->%d) ",x,mincostflow::edge[i].to);
		}
	}
}
int main() {
	...
	FOR(i,1,n) FOR(j,1,m) {
		...
		scanf("%d",&op);
		if (op==1) continue;
		if (op==2) addedge(getid(i,j),n*m+getid(i,j),1,-1);
		addedge(getid(i,j),n*m+getid(i,j),INF,0);
		if (i+1<=n) addedge(getid(i,j)+n*m,getid(i+1,j),INF,0);
		if (j+1<=m) addedge(getid(i,j)+n*m,getid(i,j+1),INF,0);
	} addedge(s,getid(1,1),k,0);
	addedge(getid(n,m),t,k,0);
	auto now=mincostflow::mincostflow(s,t,t,k);
	k=now.first;
	FOR(cnt,1,k) dfs(s);
}
/*
