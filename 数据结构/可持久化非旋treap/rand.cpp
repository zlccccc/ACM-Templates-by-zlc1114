...
namespace persist_treap {
    ...
    int build(int A[],int l,int r) {
        if (l>r) return 0;
        if (l==r) return insnode(A[l]);
        int mid=(l+r)/2;
        int ret=merge(build(A,l,mid),build(A,mid+1,r));
        return ret;
    }
...
int main() {
    ...
    FOR(_,1,T){
        ...
        // printf("LEN=%d\n",persist_treap::T[persist_treap::root].len);
